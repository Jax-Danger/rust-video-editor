...
The **Audio** workspace is the mixer. Each audio track has a fader (−∞ to +12 dB, unity at 0 dB), a constant-power pan (center is unity, so an unpanned clip is unchanged), a 3-band EQ (low shelf 200 Hz, mid peak 1 kHz, high shelf 8 kHz, ±12 dB, plus an optional 80 Hz low-cut), a dynamics compressor (threshold −60…0 dB, ratio 1:1…20:1, attack 0.1…100 ms, release 10…1000 ms, makeup 0…24 dB), mute, solo, and peak / RMS / peak-hold meters. Double-click an EQ band or dynamics control to reset it to the default. A red clip lamp latches when that strip, or the master, reaches full scale; click it to clear. The master fader sits on the bus after the tracks. Fader, pan, EQ, compressor, mute, solo, master, and clip gain are applied while the chunk plays, so a move is heard without waiting for the next decode. Clip gain is an `AnimatedF32`: the inspector diamond (and the one on the strip) keys it at the playhead. The slider is 0 to 2; the value is stored up to 4. EQ and compressor settings persist on each track in project JSON (`eq.*` and `compressor.*`; omitted when bypassed). Playback, the meters, caption extraction, and export all use the same signal order: **clip gain → EQ → compressor → track fader × pan → master fader → hard limit at full scale**.
