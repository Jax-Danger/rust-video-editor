...
The **Audio** workspace is the mixer. Each audio track has a fader (−∞ to +12 dB, unity at 0 dB), a constant-power pan (center is unity, so an unpanned clip is unchanged), a 3-band EQ (low shelf 200 Hz, mid peak 1 kHz, high shelf 8 kHz, ±12 dB, plus an optional 80 Hz low-cut), a dynamics compressor (threshold −60…0 dB, ratio 1:1…20:1, attack 0.1…100 ms, release 10…1000 ms, makeup 0…24 dB), sidechain ducking, mute, solo, and peak / RMS / peak-hold meters. Double-click an EQ band, dynamics control, or duck control to reset it to the default. A red clip lamp latches when that strip, or the master, reaches full scale; click it to clear. The master fader sits on the bus after the tracks. Fader, pan, EQ, compressor, duck, mute, solo, master, and clip gain are applied while the chunk plays, so a move is heard without waiting for the next decode. Clip gain is an `AnimatedF32`: the inspector diamond (and the one on the strip) keys it at the playhead. The slider is 0 to 2; the value is stored up to 4. EQ, compressor, and duck settings persist on each track in project JSON (`eq.*`, `compressor.*`, and `duck.*`; omitted when bypassed). Playback and the meters use **clip gain → EQ → compressor → duck → track fader × pan → master fader → hard limit at full scale**.

**Duck when** another audio track peaks above a threshold. On the strip, turn **ON**, pick the source track (dialogue, for a music bed), and set threshold (−60…0 dB, default −20), amount (0…−24 dB, default −12), attack (0.1…200 ms), and release (10…2000 ms). While the source peak is above the threshold the track falls by the full amount; attack and release smooth that gain. A muted or non-solo source does not duck, and a track cannot duck itself. The reduction runs in the shared mix, so the strip meters show the ducked level. Export inserts ffmpeg `sidechaincompress` from that source track with the same threshold, attack, and release. Its ratio is chosen so a full-scale source settles at the amount; playback applies the full amount as soon as the threshold is crossed.
...
**Stabilize** is a warp-stabilizer-lite: it estimates 2D motion with a centre block match between consecutive source frames, low-passes the cumulative path with the smoothing slider, then applies the inverse shift (and optional rotation from baked analysis) in the shared compositor for preview and Deliver. With no baked keyframes, motion is tracked causally as frames are decoded during playback or export. You can also persist a pre-analyzed path on the clip (`keyframes` in project JSON) or in a sidecar file next to the media (`<file>.stabilize.json`).
...
- **Deliver** — export presets (YouTube, Shorts, Instagram, ProRes master, H.265, audio WAV), codec/container, in/out, **Export**, and still export at the playhead or at every marker. With `--features ffmpeg` this encodes a real file. Without that feature, Export still writes the JSON manifest and says the encoder is compiled out.
...
- Audio uses the mixer bus: mute and solo, clip gain (with keyframes), pre-fader track EQ, pre-fader track compressor, track fader, constant-power pan, then the master fader. Offline audio is skipped and named in the report. Offline video that is actually visible fails the export.
